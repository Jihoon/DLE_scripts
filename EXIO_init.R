...
# From IoT folder
L_inverse <- read.table(paste(path_iot, "L_inverse.txt", sep=""), header=F, sep=",", dec=".")
factor_input <- read.table(paste(path_iot, "mrFactorInputs_version2.2.2.txt", sep=""), header=F, sep="\t", dec=".", skip=2)
final_demand <- read.table(paste(path_iot, "mrFinalDemand_version2.2.2.txt", sep=""), header=F, sep="\t", dec=".", skip=2)
iot <- read.table(paste(path_iot, "mrIot_version2.2.2.txt", sep=""), header=F, sep="\t", dec=".", skip=2)
...
materials <- read.table(paste(path_iot, "mrMaterials_version2.2.0.txt", sep=""), header=F, sep="\t", dec=".", skip=2)
final_demand_material <- read.table(paste(path_iot, "mrFDMaterials_version2.2.0.txt", sep=""), header=F, sep="\t", dec=".", skip=2)

# From SUT folder
tot_use <- read.table(paste(path_sut, "mrUse_version2.2.2.txt", sep=""), header=F, sep="\t", dec=".", skip=2)
...
energy_use <- materials[energy_carrier_use_idx,]
y <- 1/tot_demand
y[is.infinite(y)] <- 0 
energy_int <- as.matrix(energy_use) %*% diag(y)
indirect_E_int <- energy_int %*% as.matrix(L_inverse)
